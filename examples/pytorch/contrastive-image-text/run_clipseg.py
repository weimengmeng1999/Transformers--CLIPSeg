...
from torchvision.transforms import CenterCrop, ConvertImageDtype, Normalize, Resize, transforms
...
class DataTrainingArguments:
    ...
    label_column:Optional[str] = field(
        default="label",
        metadata={"help": "The name of the column in the datasets containing the label file path."},
    )
...
def main():
    ...
    send_example_telemetry("run_clipseg", model_args, data_args)
    ...
    #processor=CLIPSegProcessor.from_pretrained(model_args.model_name_or_path)
    ...
    if data_args.label_column is None:
        label_column = dataset_columns[2] if dataset_columns is not None else column_names[2]
    else:
        label_column = data_args.label_column
        if label_column not in column_names:
            raise ValueError(
                f"--label_column' value '{data_args.label_column}' needs to be one of: {', '.join(column_names)}"
            )
    ...
    def transform_images(examples):
        ...
        orilabels=[read_image(label_file) for label_file in examples[label_column]]
        examples["labels"]=[torch.squeeze(label_transformations(orilabel),dim=0).to(torch.float) for orilabel in orilabels]
    ...
    if training_args.do_train:
        ...
        train_dataset = train_dataset.map(
            ...
            batched=True,
            remove_columns=[col for col in column_names if (col != image_column and col!= label_column)],
